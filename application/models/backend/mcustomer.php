...
class mcustomer extends MY_Admin_Controller
{
    ...
    public function __construct()
    {
		...
		$this->load->Model("backend/muser.php") ;
    }
    ...
    public function add()
    {
        $this->muser->permision("user","add");
        $this->_data["title"] = "Thêm m?i User";
        $this->_data['disabled'] = '';
        ...
        $this->form_validation->set_rules('user_username', 'Tài kho?n', 'required|trim|min_length[5]|max_length[32]');
        $this->form_validation->set_rules('user_password', 'M?t kh?u', 'required|trim|min_length[8]|max_length[32]');
    ...
    {
        $this->muser->permision("user","update");
        $this->_data["title"] = "Thêm m?i User";
        ...
        $this->_data["formData"]["user_updatedate"]       =   time();
        $this->form_validation->set_error_delimiters('<div class="alert alert-danger" style="padding:5px;margin:0;border:0;border-radius:0">', '</div>');
        $this->form_validation->set_rules('user_password', 'M?t kh?u', 'required|trim|min_length[8]');
        $this->form_validation->set_rules('user_first_name', 'Tên', 'required|trim');
        $this->form_validation->set_rules('user_email', 'Email', 'required|trim|valid_email');
        if ($this->form_validation->run() == TRUE) {
            $user_birthday = $this->input->post("user_birthday");
    ...
    public function update_status($id, $status, $path='')
    {
        ...
        if (isset($id) && is_numeric($id)) {
            ...
            $this->_data["title"] = "Không tìm th?y thông tin c?p nh?t";
            $this->_data["message"] = "Vui lòng ki?m tra l?i du?ng d?n !!";
    ...
    public function profile()
    {
        ...
        $this->_data['title'] = 'C?p nh?t thông tin';
