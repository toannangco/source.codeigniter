            ...
            <div class="box box-primary">
                <div class="box-header">
                    ...
                    <div class="pull-right box-tools">
                        <a href="<?= admin_url ?>news/add/<?= $news_parent >0 ? '?news_parent='.$news_parent:'';?>" class="btn btn-primary btn-sm" title="<?= them?> <?= bai_viet?>">
                ...
                <div id="example1_wrapper" class="box-body" role="grid">
                    <div class="row">
                        <div class="form-group col-md-3">
                            <select onchange="this.form.submit()" class="form-control input-sm" name="news_parent" id="news_parent">
                                <option value="">---<?= loc_theo?> <?= danh_muc?>---</option>
                                <?php 
                                    $this->mmenu->dropDownMenu($news_parent,'news'); 
                                ?>
                            </select>
                        </div>
                        ...
                        <table id="example1" class="table table-bordered table-striped dataTable table-checkbox" aria-describedby="example1_info">
                            <thead>
                                <tr role="row">
                                    ...
                                    <th><?= danh_muc?></th>
                                    <th><?= hinh_dai_dien?></th>
                                    <th class="text-center"><?= luot_xem?></th>
                                    <th><?= tac_gia?></th>
                            ...
                            if(isset($list)){
                                foreach ($list as $key => $value) {
                                    $myMenuLang = $this->mmenu_lang->getDataObject('menu_lang_name as menu_name',array("menu_lang"=>$lang,"menu_id"=>$value->news_parent));
                                    $menu_name = !empty($myMenuLang) ? $myMenuLang->menu_name : '--';
                                    
                                    $picture = $value->news_picture!=""?base_file.'news/'.$value->news_picture:admin_img.'no_image.png';
                                    ...
                                    $link_edit = admin_url.'news/update/'.$value->id.'/?redirect='.base64_encode(curPageURL());
                                    $link_status = admin_url.'news/status/'.$value->id.'/'.$status_change.'/?redirect='.base64_encode(curPageURL());
                                    $link_delete = admin_url.'news/status/'.$value->id.'/-1/?redirect='.base64_encode(curPageURL());
                                    if($value->news_status == -1)
                                        $link_delete = admin_url.'news/delete/'.$value->id.'/?redirect='.base64_encode(curPageURL());
                                    ?>
                                    <tr class="odd">
                                        <td class="text-center">
                                            <?= $value->id?>
                                        </td>
                                        <td><?= $value->news_lang_name?></td>
                                        <td><?= $menu_name?></td>
                                        <td class="text-center"><a href="<?= $picture?>" class="image zoom_img tip-bottom" title="Zoom img"><img src="<?= $picture?>" class="img-circle"></a></td>
                                        <td class="text-center"><small class="badge bg-aqua"><?= $value->news_view?></small></td>
                                        <td><small class="text-gray"><?= $authorName?></small></td>
                                        <td class="text-center"><small><?= $value->news_update_date?></small></td>
                                        <td class="text-center">
